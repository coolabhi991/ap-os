...
import type { EmailRecipient } from "../../services/banking-reports";
...
export default function EmailReportModal({ title, onClose, onSend }: Props) {
  const [recipients, setRecipients] = useState<EmailRecipient[]>([{ label: "", email: "" }]);
  const [message, setMessage] = useState("");
  const [sending, setSending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const addRow = () => setRecipients((rows) => [...rows, { label: "", email: "" }]);
  const updateRow = (i: number, patch: Partial<EmailRecipient>) => setRecipients((rows) => rows.map((r, idx) => (idx === i ? { ...r, ...patch } : r)));
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await onSend(recipients, message || undefined);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-lg">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold">Email {title}</h2>
          ...
          <div className="space-y-3">
            {recipients.map((r, i) => (
              <div key={i} className="flex items-center gap-2">
                <input type="email" placeholder="Email address" value={r.email} onChange={(e) => updateRow(i, { email: e.target.value })} required className="flex-1 rounded-lg border p-2" />
                <input type="text" placeholder="Name (optional)" value={r.label} onChange={(e) => updateRow(i, { label: e.target.value })} className="flex-1 rounded-lg border p-2" />
