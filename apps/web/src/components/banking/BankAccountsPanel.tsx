...
} from "../../services/bank-account-masters";
import type { BankAccountMaster, BankAccountMasterFormData, BankAccountOwnerType } from "../../services/bank-account-masters";
...
function emptyForm(ownerType: BankAccountOwnerType, ownerId: string): BankAccountMasterFormData {
  return {
    ownerType,
    ownerId,
    ownerLabel: "",
...
/** Reusable "unlimited bank accounts per owner" panel, backed by the Bank Account Master — used
 * for any owner type that doesn't already have its own dedicated bank-account flow (Company and
 * Vendor keep their existing CompanyBankAccount/VendorBankAccount screens). */
export default function BankAccountsPanel({ ownerType, ownerId }: Props) {
  const [accounts, setAccounts] = useState<BankAccountMaster[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<BankAccountMasterFormData>(emptyForm(ownerType, ownerId));
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      setAccounts(await getBankAccountMasters(ownerType, ownerId));
    } catch {
  ...
  const startAdd = () => {
    setEditingId(null);
    setForm(emptyForm(ownerType, ownerId));
    setFormError(null);
    setShowForm(true);
  };

  const startEdit = (account: BankAccountMaster) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      setFormError(null);
      if (editingId) await updateBankAccountMaster(editingId, form);
      else await createBankAccountMaster(form);
      setShowForm(false);
      await load();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteBankAccountMaster(id);
      await load();
    } catch (err) {
  ...
  return (
    <div className="rounded-xl bg-white p-6 shadow-sm">
      <div className="mb-5 flex items-center justify-between">
        ...
        <button onClick={startAdd} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">
      ...
      {!loading && !error && (
        <div className="space-y-3">
          {accounts.length === 0 && !showForm && <p className="text-sm text-slate-500">No bank accounts on file.</p>}
          {accounts.map((a) => (
            <div key={a.id} className={`rounded-lg border p-4 ${a.status === "ACTIVE" ? "border-slate-200" : "border-slate-100 bg-slate-50 opacity-70"}`}>
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${BANK_ACCOUNT_STATUS_COLORS[a.status]}`}>
                      {BANK_ACCOUNT_STATUS_LABELS[a.status]}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-slate-500">
                    {a.accountHolder && <>{a.accountHolder} • </>}
                    {maskAccountNumber(a.accountNumber)} • {a.ifscCode}
                  </p>
                  {(a.branch || a.upiId) && (
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4 rounded-lg border border-slate-200 p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input value={form.accountHolder} onChange={(e) => setForm({ ...form, accountHolder: e.target.value })} className="w-full rounded-lg border p-2.5" />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.upiId} onChange={(e) => setForm({ ...form, upiId: e.target.value })} placeholder="name@upi" className="w-full rounded-lg border p-2.5" />
            </div>
            <div>
              ...
              <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="w-full rounded-lg border p-2.5">
                {BANK_ACCOUNT_STATUS_OPTIONS.map((s) => <option key={s} value={s}>{BANK_ACCOUNT_STATUS_LABELS[s]}</option>)}
              </select>
            </div>
