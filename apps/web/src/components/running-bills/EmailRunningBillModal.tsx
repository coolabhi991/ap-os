...
import { sendRunningBillEmail, EMAIL_RECIPIENT_ROLES, EMAIL_RECIPIENT_ROLE_LABELS } from "../../services/running-bills";
import type { EmailRecipient } from "../../services/running-bills";
...
export default function EmailRunningBillModal({ runningBillId, onClose, onSent }: Props) {
  const [recipients, setRecipients] = useState<RecipientRow[]>([{ role: "CLIENT", label: "", email: "" }]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await sendRunningBillEmail(runningBillId, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-lg">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold">Email Running Bill</h2>
