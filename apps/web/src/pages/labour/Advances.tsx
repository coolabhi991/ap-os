...
import AdvanceTable from "../../components/labour/AdvanceTable";
import { getLabourAdvances, deleteLabourAdvance, exportLabourAdvancesCSV } from "../../services/labour-advances";
...
export default function Advances() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const result = await getLabourAdvances(query);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteLabourAdvance(id);
      setAdvances((prev) => prev.filter((a) => a.id !== id));
    } catch (err) {
      alert(err instanceof Error ? err.message : "Failed to delete advance.");
    }
  };
  ...
  const handleExport = async () => {
    try {
      setExporting(true);
      await exportLabourAdvancesCSV(query);
    } catch {
      alert("Failed to export advances.");
    } finally {
      setExporting(false);
    }
  };
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Labour Advances</h1>
            <p className="mt-2 text-slate-500">Cash draws against future wages.</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => navigate("/labour/advances/new")} className="flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-3 text-white hover:bg-blue-700">
        ...
        {!loading && !error && <AdvanceTable advances={advances} onDelete={handleDelete} />}
      </div>
    </Layout>
  );
}
