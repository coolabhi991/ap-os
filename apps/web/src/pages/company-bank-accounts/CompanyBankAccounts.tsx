...
const emptyForm: CompanyBankAccountFormData = {
  nickname: "",
  beneficiaryName: "",
  bankName: "",
  accountNumber: "",
...
export default function CompanyBankAccounts() {
  ...
  const [form, setForm] = useState<CompanyBankAccountFormData>(emptyForm);
  ...
  const load = async () => {
    try {
      ...
      setError("Failed to load company bank accounts.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const startEdit = (account: CompanyBankAccount) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(null);
      if (editingId) {
        await updateCompanyBankAccount(editingId, form);
      } else {
        await createCompanyBankAccount(form);
      }
      setShowForm(false);
      await load();
    } catch (err) {
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Company Bank Accounts</h1>
            <p className="mt-2 text-slate-500">Manage the accounts your company pays vendors from.</p>
          </div>
          <button
            onClick={startAdd}
          ...
          <div className="space-y-3">
            {accounts.length === 0 && !showForm && (
              <div className="rounded-xl border bg-white py-16 text-center text-slate-500 shadow-sm">
                No company bank accounts on file yet.
              </div>
            )}
            {accounts.map((a) => (
              <div key={a.id} className={`rounded-xl border bg-white p-5 shadow-sm ${!a.isActive ? "opacity-60" : ""}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => startEdit(a)}><Pencil size={18} className="text-green-600" /></button>
                    <button onClick={() => handleDelete(a.id)}><Trash2 size={18} className="text-red-600" /></button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="space-y-6 rounded-xl bg-white p-8 shadow-sm">
            ...
            <div className="flex justify-end gap-4">
              <button type="button" onClick={() => setShowForm(false)} className="rounded-lg border px-6 py-3">Cancel</button>
