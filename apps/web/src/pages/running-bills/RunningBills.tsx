      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => navigate("/running-bills/reports")} className="flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm hover:bg-slate-50">
            ...
            >
              <Plus className="h-4 w-4" /> New Running Bill
            </button>
          </div>
        </div>
        ...
        {!loading && !error && (
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <table className="min-w-full">
              ...
              <tbody>
                {bills.length === 0 ? (
                  <tr><td colSpan={9} className="py-10 text-center text-slate-500">No Running Bills raised yet.</td></tr>
                ) : (
                  bills.map((b) => (
                    <tr key={b.id} className="cursor-pointer border-t hover:bg-slate-50" onClick={() => navigate(`/running-bills/${b.id}`)}>
                      ...
                      <td className="px-4 py-3">{BILL_TYPE_LABELS[b.billType] ?? b.billType}</td>
                      <td className="px-4 py-3 text-right">₹{Number(b.netPayable).toLocaleString("en-IN")}</td>
                      <td className="px-4 py-3 text-right">₹{Number(b.amountReceived).toLocaleString("en-IN")}</td>
                      <td className="px-4 py-3 text-right font-medium text-amber-600">₹{Number(b.outstandingAmount).toLocaleString("en-IN")}</td>
