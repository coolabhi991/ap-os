        ...
        {!loading && !error && (
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <table className="min-w-full">
              ...
              <tbody>
                {mbs.length === 0 ? (
                  <EmptyTableRow colSpan={9}>No Measurement Books recorded yet.</EmptyTableRow>
                ) : (
                  mbs.map((m) => (
                    <tr key={m.id} className="cursor-pointer border-t hover:bg-slate-50" onClick={() => navigate(`/measurement-books/${m.id}`)}>
                      <td className="px-6 py-4 font-medium">{m.mbNumber}</td>
                      <td className="px-6 py-4">{m.mbDate}</td>
                      <td className="px-6 py-4">{m.project?.name ?? "—"}</td>
                      <td className="px-6 py-4">{m.subWork?.name ?? "—"}</td>
                      <td className="px-6 py-4">{m.contractor?.name ?? "—"}</td>
                      <td className="px-6 py-4 text-right">{formatIndianNumber(m.totalQuantity)}</td>
                      <td className="px-6 py-4 text-right">{formatCurrency(m.totalAmount)}</td>
