...
- **`apps/web/src/components/ui/`** (as of the Phase 4 production-readiness pass): a set of hand-rolled shared primitives — `LoadingState`, `EmptyTableRow`, `ErrorBanner` — matching the look already established across the app (`rounded-xl border border-slate-200 bg-white ... shadow-sm`). **Correction to prior docs**: this folder previously held a full parallel design system (`Card`, `Button`, `Badge`, `Table`, `PageHeader`, `Section`, a second `StatCard`) that was never imported anywhere and has since been deleted as dead code — there never were real shadcn/radix components here. Reuse the current three primitives for new loading/empty/error states rather than re-inlining the boxes by hand.
- `apps/web/src/lib/utils.ts` also exports `formatCurrency`, `formatIndianNumber`, `todayISO`, and `formatDate` (added in the Phase 4 pass to replace ~40 files' worth of locally-redefined `` `₹${Number(v).toLocaleString("en-IN")}` `` / `new Date().toISOString().slice(0, 10)` snippets) — use these instead of redefining a local formatter in a new file. `cn()` (clsx + tailwind-merge) remains the standard way to conditionally compose class names.
